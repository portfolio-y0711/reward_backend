...
export const AddReviewActionHandler = (db: IEventDatabase) => {
  return async (eventInfo: IReviewPointEvent) => {
    ...
    if (isRewardable) {
      ...
      const rewardParam = [uuidv4(), eventInfo['userId'], eventInfo['reviewId'], operation, totalpoint, reason]

      const transactionCmds: any[] = [
        [`INSERT INTO USERS_REWARDS(rewardId,userId,reviewId,operation,pointDelta,reason) VALUES(?, ?, ?, ?, ?, ?);` , ...rewardParam],
